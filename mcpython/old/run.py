...
"""
shader = pyglet.gl.current_context.create_program((default_vert_src, 'vertex'), (default_frag_src, 'fragment'))
...
def cube_vertices(x, y, z, n):
    ...
    """
    v = x, y, z
    return [v[i % 3] + x * n for i, x in enumerate(vertices)]
...
def tex_coord(x, y, n=4):
    ...
    dx = x * m
    dy = y * m
    return dx, dy, dx + m, dy, dx + m, dy + m, dx, dy, dx + m, dy, dx, dy + m
...
class Model(object):
    ...
    def _show_block(self, position, texture):
        ...
        # create vertex list
        shader_group = pyglet.graphics.ShaderGroup(shader, parent=self.group)
        self._shown[position] = shader.vertex_list(
            36, GL_TRIANGLES, self.batch, shader_group,
...
class Window(pyglet.window.Window):
    ...
    def set_3d(self):
        ...
        """
        self.projection = Mat4.perspective_projection(self.aspect_ratio, z_near=0.1, z_far=100, fov=45)
    ...
    def draw_label(self):
        ...
        self.label.text = '%02d (%.2f, %.2f, %.2f) %d / %d' % (
            (-1 if True else pyglet.clock.get_fps()), x, y, z,
